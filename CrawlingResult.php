...
if($_SESSION['admin']){
	...
	if($_POST['hitpit'] == 1){
		...
		$table_head = '<tr><th>name</th><th>name_detail</th><th>address</th><th>years</th><th>player_code</th><th>num</th><th>player_name</th><th>team_name</th><th>at_game</th><th>win</th><th>lose</th><th>hold</th><th>save</th><th>pc</th><th>inning</th><th>er</th><th>rs</th><th>hit</th><th>hr</th><th>bb</th><th>hbp</th><th>so</th></tr>';
		$position = '투수';
        $hitpit = 'PIT_';
		/*
		$table_body .= "<tr><td>".$league_name."</td>
...
?>
<body>
    <h6 class='text-right' style='margin-right:20px;color:#515151'><a href='main.html'>Main</a> > <a href='crawling.html'>Crawling</a> > Result</h6>
	<div class="table-responsive-sm">
		<table class="table table-bordered first_table" style="width:100%">
			<tr><th colspan='4' style="width:100%;text-align:center;font-size:140%">리그 정보</th></tr>
			<tr><th style='width:25%'>리그 명</th><td style='width:25%'><?=$league_name?></td><th style='width:25%'>세부 리그 명</th><td style='width:25%'><?=$league_detail?></td></tr>
			<tr><th>기록 구분</th><td><?=$position?></td><th>리그 년도</th><td><?=$_POST['league_year']?>년</td></tr>
			<tr><th>리그 URL</th><td><a href="<?=$league_url?>" target="_blank">바로가기</a></td><th>기록 URL</th><td><a href="<?=$main_url?>" target="_blank">바로가기</a></td></tr>
            <tr><th style='vertical-align: middle'>다운로드</th>
                <td style='vertical-align: middle'>
                    <div class='row'>
                        <div class='col-2'>
                            <form action='download_xls.php' method="post">
                                <input type='hidden' name='league_name' value='<?=$_POST['league_name']?>'>
                                <input type='hidden' name='league_name_detail' value='<?=$_POST['league_name_detail']?>'>
                                <input type='hidden' name='league_year' value='<?=$_POST['league_year']?>'>
                                <input type='hidden' name='hitpit' value='<?=$_POST['hitpit']?>'>
                                <input type='hidden' name='file_names' value='<?=$hitpit.$league_detail.'_'.$_POST['league_year'].'.xls'?>'>
                                <input class="btn btn-primary" type="submit" value="xls" style='padding-top:0px;padding-bottom:0px'>
                            </form>
                        </div>
                        <div class='col-2'>
                            <form action='download_csv.php' method="post">
                                ...
                                <input type='hidden' name='league_year' value='<?=$_POST['league_year']?>'>
                                <input type='hidden' name='hitpit' value='<?=$_POST['hitpit']?>'>
                                <input type='hidden' name='file_names' value='<?=$hitpit.$league_detail.'_'.$_POST['league_year'].'.csv'?>'>
                                <input class="btn btn-primary" type="submit" value="csv" style='padding-top:0px;padding-bottom:0px'>
                            </form>
                        </div>
                    </div>
                </td>
                <th style='vertical-align: middle'>DB저장</th>
                <td style='vertical-align: middle'>
                    <form action='upload_crawling.php' method="post">
                        <input type='hidden' name='league_name' value='<?=$_POST['league_name']?>'>
                        <input type='hidden' name='league_name_detail' value='<?=$_POST['league_name_detail']?>'>
                        <input type='hidden' name='league_year' value='<?=$_POST['league_year']?>'>
                        <input type='hidden' name='hitpit' value='<?=$_POST['hitpit']?>'>
                        <input class="btn btn-primary" type="submit" value="save" style='padding-top:0px;padding-bottom:0px'>
                    </form>
                </td>
            </tr>
		</table>
